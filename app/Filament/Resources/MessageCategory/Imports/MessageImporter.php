<?php

namespace App\Filament\Resources\MessageCategory\Imports;

use App\Filament\Resources\MessageCategory\Model\Message;
use Filament\Actions\Imports\ImportColumn;
use Filament\Actions\Imports\Importer;
use Filament\Actions\Imports\Models\Import;

class MessageImporter extends Importer
{
    protected static ?string $model = Message::class;

    public static function getColumns(): array
    {
        return [

            ImportColumn::make('category_id')
                    ->requiredMapping()
                    ->rules(['required'])
                    ->integer()
                    ->examples(['4', '3']),

            ImportColumn::make('message')
                ->requiredMapping()
                ->rules(['required'])
                ->examples(['Messag Category', 'required']),

            ImportColumn::make('translation_value:en')
                ->requiredMapping()
                ->rules(['required', 'max:255'])
                ->examples(['Messag Category', ':attribute is required']),

            ImportColumn::make('translation_value:ar')
                ->requiredMapping()
                ->rules(['required', 'max:255'])
                ->examples(['أنواع الرسائل', ':attribute لا يمكن ان يكون فارغا']),


        ];
    }

    public function resolveRecord(): ?Message
    {
        // return Page::firstOrNew([
        //     // Update existing records, matching them by `$this->data['column_name']`
        //     'email' => $this->data['email'],
        // ]);

        return new Message();
    }

    public static function getCompletedNotificationBody(Import $import): string
    {
        $body = 'Your Message import has completed and ' . number_format($import->successful_rows) . ' ' . str('row')->plural($import->successful_rows) . ' imported.';

        if ($failedRowsCount = $import->getFailedRowsCount()) {
            $body .= ' ' . number_format($failedRowsCount) . ' ' . str('row')->plural($failedRowsCount) . ' failed to import.';
        }

        return $body;
    }
}
