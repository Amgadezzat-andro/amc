...
use Filament\Actions\Imports\Jobs\ImportCsv as JobsImportCsv;
...
class CustomImportCsv extends JobsImportCsv implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach (($rows ?? $this->rows) as $row) {
            ...
            try {
                ...
                Notification::make()
                    ->title(collect($exception->errors())->flatten()->implode(' '))
                    ->danger()
                    ->send();
